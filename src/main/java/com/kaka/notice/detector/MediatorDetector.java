...
public class MediatorDetector implements IDetector {
    ...
    public boolean discern(Class<?> cls) {
        ...
        Class<? extends Mediator> mediatorClass = (Class<? extends Mediator>) cls;
        Facade facade = sc.context().isEmpty() ? FacadeFactory.getFacade() : FacadeFactory.getFacade(sc.context());
        Mediator observer = facade.registerMediator(mediatorClass);
        if (facade.hasCommand("print_log")) {
            facade.sendMessage(new Message("print_log", new Object[]{MediatorDetector.class, new Object[]{observer.getName(), mediatorClass}}));
        }
        return true;
    }
